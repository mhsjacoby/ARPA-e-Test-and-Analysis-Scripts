...
class DynamicUpdatePlot():
    ...
    def plot_lines(self):
        ...
        for x,y in zip(self.times[0:self.max], self.light_data[0:self.max]):
           self.x.append(x), self.y.append(y) 
        print(self.t_start)

        plt.xlabel('Elapsed Time (s)')
        plt.ylabel('Light (lux)')
        #plt.title('lighterature')	

        line1, = ax1.plot(self.x, self.y, 'b-')
        ax1.set_ylim(200, 8000)

        for x,y in zip(self.times[self.max:], self.light_data[self.max:]):
            self.x.append(x), self.y.append(y)
            print(list(self.x))
            print(list(self.y))
