...
for station_num in station_nums:
	target_files = "H%s/%sS%s/0_complete_csv/H*"%(H_num, station_color, station_num) # Change this to include the root path to read the csvs
	...
	np.savetxt(folder_name+"/H%s_%sS%s_stacked.csv"%(H_num, station_color, station_num), main_data, delimiter=',',fmt='%s',header=headers,comments='')
	...
	np.savetxt(folder_name+"/H%s_%sS%s_stacked_avg.csv"%(H_num, station_color, station_num), main_avg_data, delimiter=',',fmt='%s',header=headers,comments='')
